...
CyberStrike 也提供桌面版應用程式。您可以直接從 [發佈頁面 (releases page)](https://github.com/CyberStrikeus/CyberStrike/releases) 或 [cyberstrike.us/download](https://cyberstrike.us/download) 下載。
...
了解更多關於 [Agents](https://cyberstrike.us/docs/agents) 的資訊。

### 線上文件

關於如何設定 CyberStrike 的詳細資訊，請參閱我們的 [**官方文件**](https://cyberstrike.us/docs)。
...
- 100% 開源。
- 不綁定特定的服務提供商。雖然我們推薦使用透過 [CyberStrike Zen](https://cyberstrike.us/zen) 提供的模型，但 CyberStrike 也可搭配 Claude, OpenAI, Google 甚至本地模型使用。隨著模型不斷演進，彼此間的差距會縮小且價格會下降，因此具備「不限廠商 (provider-agnostic)」的特性至關重要。
- 內建 LSP (語言伺服器協定) 支援。
