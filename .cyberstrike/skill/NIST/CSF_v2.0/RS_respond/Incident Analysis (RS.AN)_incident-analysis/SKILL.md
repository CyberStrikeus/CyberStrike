---
name: "Incident Analysis (RS.AN)_incident-analysis"
description: "Investigations are conducted to ensure effective response and support forensics and recovery activities"
category: "configuration"
version: "2.0"
author: "cyberstrike-official"
tags:
  - nist
  - csf
  - v2.0
  - incident analysis (rs-an)
  - rs
  - category
tech_stack:
  - any
cwe_ids:
  []
chains_with:
  []
prerequisites:
  []
severity_boost: {}
---

# Incident Analysis (RS.AN) Incident Analysis

## High-Level Description

**Function:** RESPOND (RS)
**Framework:** NIST Cybersecurity Framework v2.0

Investigations are conducted to ensure effective response and support forensics and recovery activities

## What to Check

- [ ] Verify Incident Analysis (RS.AN) Incident Analysis outcome is achieved
- [ ] Review documentation and evidence for Incident Analysis (RS.AN)
- [ ] Assess organizational maturity for RESPOND function

## How to Test

### Step 1: Identify Current Profile

Determine the organization's current and target CSF profile tier for Incident Analysis (RS.AN).

### Step 2: Assess Outcome Achievement

```
# Review organizational policies and procedures
# Check for evidence that Incident Analysis (RS.AN) outcome is met
# Interview stakeholders responsible for RESPOND
```

### Step 3: Map to Technical Controls

Identify which SP 800-53 controls implement this CSF outcome and verify their operating effectiveness.

## Tools

| Tool | Purpose | Usage |
| ---- | ------- | ----- |
| cloud-audit-mcp | Assess cloud security posture | `cloud_audit_*` tools |
| Manual Review | Policy and procedure review | Interviews and documentation |

## Remediation Guide

Achieve the Incident Analysis (RS.AN) Incident Analysis outcome:

Investigations are conducted to ensure effective response and support forensics and recovery activities

## Risk Assessment

| Finding | Severity | Impact |
| ------- | -------- | ------ |
| Incident Analysis (RS.AN) Incident Analysis outcome not achieved | Medium | RESPOND Function Gap |

## CWE Categories

| CWE ID | Title |
| ------ | ----- |
| N/A | No direct CWE mapping |

## References

- [NIST CSF v2.0](https://www.nist.gov/cyberframework)
- [NIST CSF v2.0 Reference Tool](https://csrc.nist.gov/projects/cybersecurity-framework/filters#/csf/filters)
- [CSF 2.0 Quick Start Guides](https://www.nist.gov/cyberframework/getting-started)

## Checklist

- [ ] Current profile tier assessed
- [ ] Target profile tier defined
- [ ] Gap analysis completed
- [ ] SP 800-53 control mapping verified
- [ ] Implementation roadmap exists
