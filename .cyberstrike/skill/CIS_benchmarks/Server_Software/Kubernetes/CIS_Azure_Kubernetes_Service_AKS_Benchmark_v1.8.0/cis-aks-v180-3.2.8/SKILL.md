...
Additionally, ensure the kubelet service file `/etc/systemd/system/kubelet.service.d/10-kubelet-args.conf` does not set the `--RotateCertificate` executable argument to false because this would override the Kubelet config file.
