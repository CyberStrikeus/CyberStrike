...
# CIS Red Hat OpenShift Container Platform Benchmark v1.7.0 - Control 1.4.1
...
Validate that RBAC is enabled and protects controller endpoints. First, switch to the `openshift-kube-scheduler` namespace:

```bash
oc project openshift-kube-scheduler
```

Next, get the scheduler pod name and port:
