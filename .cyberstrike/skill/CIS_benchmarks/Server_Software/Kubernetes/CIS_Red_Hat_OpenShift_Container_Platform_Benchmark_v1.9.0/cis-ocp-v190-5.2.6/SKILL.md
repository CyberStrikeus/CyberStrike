...
# CIS Red Hat OpenShift Container Platform Benchmark v1.9.0 - Control 5.2.6
...
```bash
oc get scc -A -o json | jq '.items[] | select(.runAsUser["type"] == "MustRunAsNonRoot") | .metadata.name'
```
