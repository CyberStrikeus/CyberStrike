...
# CIS Red Hat OpenShift Container Platform Benchmark v1.9.0 - Control 5.7.4
...
For more information, see Working with projects and Configuring project creation.
...
```bash
oc get all -n default -o json | jq '.items[] | select((.kind|test("Service")) and (.metadata.name|test("openshift|kubernetes"))? | not) | (.kind + "/" + .metadata.name)'
```

There are 15 separate resource categories returned in the json dump from oc get all; they all should be empty unless related to a system resource.
...
|---|---|---|---|---|
| v8 | 4.9 Configure Trusted DNS Servers on Enterprise Assets | | x | x |
