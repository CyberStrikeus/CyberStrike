...
# CIS Red Hat OpenShift Container Platform Benchmark v1.9.0 - Control 5.2.1
...
```bash
oc get scc -o json | jq '.items[] | select(.allowPrivilegedContainer==false) | .metadata.name'
```
