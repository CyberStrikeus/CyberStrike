...
# CIS Red Hat OpenShift Container Platform Benchmark v1.9.0 - Control 4.1.5
...
for node in $(oc get nodes -o jsonpath='{.items[*].metadata.name}')
do
oc debug node/${node} -- chroot /host stat -c %a /host/etc/kubernetes/kubelet.conf
done
```
