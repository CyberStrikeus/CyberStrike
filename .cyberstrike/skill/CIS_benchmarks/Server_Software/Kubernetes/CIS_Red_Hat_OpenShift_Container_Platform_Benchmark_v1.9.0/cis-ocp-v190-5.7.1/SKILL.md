...
# CIS Red Hat OpenShift Container Platform Benchmark v1.9.0 - Control 5.7.1
...
```bash
oc get namespaces -o json | jq '.items[] | select(.metadata.name|test("(?!default|kube-.|openshift.)^.*")) | .metadata.name'
```
