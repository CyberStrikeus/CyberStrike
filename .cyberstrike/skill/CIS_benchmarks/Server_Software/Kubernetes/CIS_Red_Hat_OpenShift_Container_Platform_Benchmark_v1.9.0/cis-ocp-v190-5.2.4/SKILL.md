...
# CIS Red Hat OpenShift Container Platform Benchmark v1.9.0 - Control 5.2.4
...
```bash
oc get scc -A -o json | jq '.items[] | select(.allowHostNetwork==false) | .metadata.name'
```
