...
1. For source builds with static modules, run the Apache `./configure` script without including the `mod_proxy` in the `--enable-modules=configure` script options.
   - `$ cd $DOWNLOAD_HTTPD`
   - `$ ./configure`
4. For dynamically loaded modules, comment out or remove the LoadModule directive for `mod_proxy` module and all other proxy modules from the `httpd.conf` file.
