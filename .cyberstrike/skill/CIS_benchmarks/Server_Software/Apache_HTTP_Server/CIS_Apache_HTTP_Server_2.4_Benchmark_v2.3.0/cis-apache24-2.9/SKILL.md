...
1. For source builds with static modules run the Apache `./configure` script without including the `mod_auth_basic` and `mod_auth_digest` in the `--enable-modules=configure` script options.
   - `$ cd $DOWNLOAD_HTTPD`
   - `$ ./configure`
4. For dynamically loaded modules comment out or remove the LoadModule directive for `mod_auth_basic`, and `mod_auth_digest` modules from the `httpd.conf` file.
