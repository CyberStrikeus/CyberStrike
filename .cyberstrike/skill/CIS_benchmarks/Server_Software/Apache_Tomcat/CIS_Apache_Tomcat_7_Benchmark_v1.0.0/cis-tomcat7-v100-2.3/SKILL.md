---
name: cis-tomcat7-v100-2.3
description: "Alter the Advertised server.built Date"
category: cis-tomcat
version: "1.0.0"
author: cyberstrike-official
tags: [cis, tomcat, tomcat-7, information-disclosure]
cis_id: "2.3"
cis_benchmark: "CIS Apache Tomcat 7 Benchmark v1.0.0"
tech_stack: [tomcat, java]
cwe_ids: []
chains_with: []
prerequisites: []
severity_boost: {}
---

# CIS Apache Tomcat 7 - 2.3 Alter the Advertised server.built Date (Scored)

## Profile Applicability
- Level 1

## Description
Alter the Advertised server.built Date

## Rationale
See CIS Apache Tomcat 7 Benchmark v1.0.0 for detailed rationale.

## Audit Procedure
Refer to CIS Apache Tomcat 7 Benchmark v1.0.0 Section 2.3 for audit commands.

## Remediation
Refer to CIS Apache Tomcat 7 Benchmark v1.0.0 Section 2.3 for remediation steps.

## References
- CIS Apache Tomcat 7 Benchmark v1.0.0

## Assessment Status
- **Scored**
- **Profile**: Level 1
