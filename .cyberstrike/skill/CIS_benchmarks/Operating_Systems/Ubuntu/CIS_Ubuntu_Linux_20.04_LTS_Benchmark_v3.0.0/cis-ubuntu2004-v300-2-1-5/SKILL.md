---
name: cis-ubuntu2004-v300-2-1-5
...
Unless a system is specifically designated to act as a DNS caching proxy or DHCP server, it is recommended that the service be disabled to reduce the potential attack surface.

## Audit Procedure

### Command Line

Run the following command to verify dnsmasq is not installed:
...
dnsmasq should not be installed, or if installed, the service should be stopped and masked.

## Remediation

### Command Line

Run the following commands to stop, mask, and purge dnsmasq:
...
dnsmasq is not installed by default.
