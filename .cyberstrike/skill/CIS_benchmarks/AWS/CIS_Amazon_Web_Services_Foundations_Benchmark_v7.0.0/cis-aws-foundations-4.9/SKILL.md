...
```bash
aws cloudtrail get-trail --region <region-name> --name <trail-name> \
...
## Expected Result
At least one multi-region trail has S3 object-level read event logging enabled for all buckets.
