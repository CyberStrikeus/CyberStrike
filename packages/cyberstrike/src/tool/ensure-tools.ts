...
const TOOL_INSTALL_MAP: Record<string, { check: string; install: string; description: string }> = {
  ...
  nuclei: { check: "nuclei", install: "go install -v github.com/projectdiscovery/nuclei/v3/cmd/nuclei@latest", description: "Vulnerability scanner" },
  ffuf: { check: "ffuf", install: "go install github.com/ffuf/ffuf/v2@latest", description: "Web fuzzer" },
  httpx: { check: "httpx", install: "go install -v github.com/projectdiscovery/httpx/cmd/httpx@latest", description: "HTTP toolkit" },
  subfinder: { check: "subfinder", install: "go install -v github.com/projectdiscovery/subfinder/v2/cmd/subfinder@latest", description: "Subdomain discovery" },
  amass: { check: "amass", install: "go install -v github.com/owasp-amass/amass/v4/...@master", description: "Attack surface mapping" },
  waybackurls: { check: "waybackurls", install: "go install github.com/tomnomnom/waybackurls@latest", description: "Wayback Machine URL fetcher" },
  gau: { check: "gau", install: "go install github.com/lc/gau/v2/cmd/gau@latest", description: "URL aggregator" },
  hakrawler: { check: "hakrawler", install: "go install github.com/hakluke/hakrawler@latest", description: "Web crawler" },
  katana: { check: "katana", install: "go install github.com/projectdiscovery/katana/cmd/katana@latest", description: "Next-gen crawler" },
  ...
  nikto: { check: "nikto", install: "brew install nikto || apt-get install -y nikto", description: "Web server scanner" },
  wpscan: { check: "wpscan", install: "gem install wpscan || brew install wpscan", description: "WordPress scanner" },
  gospider: { check: "gospider", install: "go install github.com/jaeles-project/gospider@latest", description: "Web spidering" },
  ...
  crlfuzz: { check: "crlfuzz", install: "go install github.com/dwisiswant0/crlfuzz/cmd/crlfuzz@latest", description: "CRLF injection scanner" },
...
export const EnsureToolsTool = Tool.define("ensure_tools", {
  ...
  parameters: z.object({
    tools: z
      .array(z.string())
      .describe(
        `Tools to check/install. Available: ${Object.keys(TOOL_INSTALL_MAP).join(", ")}`,
      ),
  }),
