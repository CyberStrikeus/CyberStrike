...
pub fn spawn_command(
    ...
    let mut envs = vec![
        (
            "CYBERSTRIKE_EXPERIMENTAL_ICON_DISCOVERY".to_string(),
            "true".to_string(),
        ),
        (
            "CYBERSTRIKE_EXPERIMENTAL_FILEWATCHER".to_string(),
            "true".to_string(),
        ),
        ("CYBERSTRIKE_CLIENT".to_string(), "desktop".to_string()),
        (
    ...
    let cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.cyberstrike/bin/cyberstrike\"".to_string(),
                "if [ ! -x \"$BIN\" ]; then".to_string(),
                format!(
                    "  curl -fsSL https://cyberstrike.us/install | bash -s -- --version {} --no-modify-path",
            ...
            let mut env_prefix = vec![
                "CYBERSTRIKE_EXPERIMENTAL_ICON_DISCOVERY=true".to_string(),
                "CYBERSTRIKE_EXPERIMENTAL_FILEWATCHER=true".to_string(),
                "CYBERSTRIKE_CLIENT=desktop".to_string(),
                "XDG_STATE_HOME=\"$HOME/.local/state\"".to_string(),
            ];
            env_prefix.extend(
                envs.iter()
                    .filter(|(key, _)| key != "CYBERSTRIKE_EXPERIMENTAL_ICON_DISCOVERY")
                    .filter(|(key, _)| key != "CYBERSTRIKE_EXPERIMENTAL_FILEWATCHER")
                    .filter(|(key, _)| key != "CYBERSTRIKE_CLIENT")
...
pub fn serve(
    ...
    let envs = [
        ("CYBERSTRIKE_SERVER_USERNAME", "cyberstrike".to_string()),
        ("CYBERSTRIKE_SERVER_PASSWORD", password.to_string()),
    ];
